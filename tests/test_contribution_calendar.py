...
def test_contribution_calendar_shows_separate_cash_park_rows_for_holdings_accounts(app, make_user):
    uid, _, _ = make_user(username="temp-cash-park-calendar", password="password123")

    with app.app_context():
        from app.models import create_temporary_contribution_plan, fetch_contribution_calendar, get_connection

        with get_connection() as conn:
            account_id = conn.execute(
                """
                INSERT INTO accounts (
                    user_id, name, wrapper_type, category, monthly_contribution, monthly_cash_park,
                    current_value, valuation_mode, cash_interest_rate, is_active
                )
                VALUES (?, 'Trading 212 ISA', 'Stocks & Shares ISA', 'ISA', 100, 25, 0, 'holdings', 0.038, 1)
                """,
                (uid,),
            ).lastrowid
            conn.commit()
        ...
        rows = {row["name"]: row for row in calendar["accounts"]}

        assert rows["Trading 212 ISA"]["component"] == "invested"
        assert rows["Trading 212 ISA"]["months"][0]["override_amount"] == 150
        assert rows["Trading 212 ISA cash park"]["component"] == "cash_park"
        assert rows["Trading 212 ISA cash park"]["months"][0]["override_amount"] == 40


def test_projection_splits_invested_growth_from_cash_park_interest(app, make_user):
    uid, _, _ = make_user(username="temp-cash-park-projection", password="password123")

    with app.app_context():
        from app.calculations import projected_account_value_at_month, projection_monthly_contribution
        from app.models import fetch_contribution_overrides, get_connection
        ...
        with get_connection() as conn:
            account_id = conn.execute(
                ...
                INSERT INTO accounts (
                    user_id, name, wrapper_type, category, monthly_contribution, monthly_cash_park,
                    current_value, uninvested_cash, valuation_mode, growth_mode, growth_rate_override,
                    cash_interest_rate, is_active
                )
                VALUES (?, 'Trading 212 ISA', 'Stocks & Shares ISA', 'ISA', 100, 50, 1000, 200, 'holdings', 'custom', 0, 0.12, 1)
                """,
                (uid,),
            ).lastrowid
            conn.commit()
        ...
        assert projection_monthly_contribution(account, None, 0) == 150.0
        assert projected_account_value_at_month(account, None, 1) == 1152.0
...
def test_contribution_calendar_allowance_frame_includes_workplace_employer_and_tax_relief(app, client, make_user):
    uid, username, password = make_user(username="calendar-workplace-gross", password="password123")
    _login(client, username, password)
    ...
    with app.app_context():
        ...
        with get_connection() as conn:
            ...
            conn.execute(
                ...
                INSERT INTO accounts (
                    user_id, name, wrapper_type, category, monthly_contribution,
                    employer_contribution, contribution_method, current_value,
                    valuation_mode, is_active
                )
                VALUES (?, 'SIPP', 'SIPP', 'Pension', 200, 0, 'standard', 0, 'manual', 1)
                """,
                (uid,),
            )
            conn.execute(
                ...
                VALUES (?, 'Workplace Pension', 'Workplace Pension', 'Pension', 133.33, 166.67, 'relief_at_source', 0, 'manual', 1)
                """,
                (uid,),
            )
            conn.commit()
