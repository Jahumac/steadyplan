...
def test_cash_park_rows_auto_appear_and_roll_up_into_account_budget_override(app, two_users):
    from app.models import fetch_budget_items, get_connection
    from app.routes.budget import _build_monthly_data, _sync_linked_override
    ...
    with app.app_context():
        ...
        items = [
            i for i in fetch_budget_items(two_users["alice"]["uid"])
            if i["linked_account_id"] == two_users["alice"]["account"]
        ]
        by_name = {item["name"]: item for item in items}

        assert "Trading 212 ISA" in by_name
        assert "Trading 212 ISA cash park" in by_name

        sections, _ = _build_monthly_data("2026-04", two_users["alice"]["uid"])
        invest_row = next(row for section in sections for row in section["rows"] if row["name"] == "Trading 212 ISA")
        cash_row = next(row for section in sections for row in section["rows"] if row["name"] == "Trading 212 ISA cash park")

        assert invest_row["amount"] == 100
        assert invest_row["source"] == "linked_account"
        assert cash_row["amount"] == 25
        assert cash_row["source"] == "linked_account"

        _sync_linked_override(by_name["Trading 212 ISA cash park"]["id"], "2026-07", 40, two_users["alice"]["uid"])
        ...
        with get_connection() as conn:
            override = conn.execute(
                ...
                WHERE account_id = ? AND from_month = '2026-07' AND to_month = '2026-07'
                """,
                (two_users["alice"]["account"],),
            ).fetchone()
            account = conn.execute(
                "SELECT monthly_cash_park FROM accounts WHERE id = ?",
                (two_users["alice"]["account"],),
            ).fetchone()

        assert float(override["override_amount"] or 0) == 140
        assert float(account["monthly_cash_park"] or 0) == 40
